...
import { showLoader, hideLoader } from "../redux/ivan/actions/appActions";
...
class CurrentUser extends React.Component {
  ...
  async сomponentDidUpdate(prevProps) {
    if (
      this.props.user.user.auctions.length !==
      prevProps.user.user.auctions.length
    ) {
  ...
  onSubmitEditHandler = (event) => {
    ...
    const userInfo = Object.fromEntries(new FormData(event.target));
  ...
  onSubmitNewAucHandler = (event) => {
    event.preventDefault();
    const newAuc = Object.fromEntries(new FormData(event.target));
    ...
    return (
      <>
        <div>
          {!this.state.editInfo && (
            <>
              <UserInfo name={user.login} description={user.description} />
              <Button
          ...
          {this.state.editInfo && (
            <>
              <EditUserInfo onSubmit={this.onSubmitEditHandler} />
            </>
          )}
        </div>
        {!this.state.addAuction ? (
          <Button text="Создать аукцион" onClick={this.onClickNewAucHandler} />
        ) : (
          <NewTender onSubmit={this.onSubmitNewAucHandler} />
        )}

        {this.state.mytenders && this.props.user.user.tenders && (
          <MyTenders tenders={this.props.user.user.tenders} />
        )}
        {this.state.myoffers && this.props.user.user.auctions && (
          <MyOffers auctions={this.props.user.user.auctions} />
        )}
        {this.state.allOffers && this.props.user.offers && (
          <>
            <p>Здесь офферы</p>
            <Offers
