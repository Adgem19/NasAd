  ...
  render() {
    return (
      <div>
        ...
        <ul>
          {this.props.state.allauction &&
            this.props.state.allauction.map((auction) => {
              return (
                <li>
                  {auction.subject}
                  {auction.budget}
                  {auction._id}
                  <button onClick={this.onChage.bind(this, auction._id)}>
...
const mapDispatchToProps = (dispatch) => {
  return {
    useraddauction: (userid, auctionid) =>
      dispatch(useraddauction(userid, auctionid)),
