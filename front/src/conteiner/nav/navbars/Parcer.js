    ...
    return (
      <div>
        ...
        <from className="form-parser">
          <input
            className="input-parser"
            type="text"
            onChange={this.onChange}
          />
          <input
            className="btn-parser"
            type="submit"
            onClick={this.onClick.bind(this, this.state.inputvalue)}
          />
        </from>
        {this.props.state.usersocial ? (
          <div>
            {" "}
            {this.props.state.usersocial &&
              this.props.state.usersocial.map((social) => {
                return (
                  <div>
                    <h2>Результаты анализа:</h2>
                    <ul>
                      <li>
                        Среднее кол-во лайков на публикации{" "}
                        <b>{social.middlelike}</b>
                      </li>
                      <li>
                        Среднее кол-во просмотров на публикации{" "}
                        <b> {social.middleview}</b>
                      </li>
                      <li>
