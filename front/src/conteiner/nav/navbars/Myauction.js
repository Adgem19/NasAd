    ...
    return (
      <div className="my-auction-content">
        <button className="btn1" onClick={this.onClick}>
          {" "}
          Просмотреть блогеров,которые откликнулись на ваше аукционы{" "}
        </button>
        {this.state.win ? (
          <div>
            {" "}
            Мы оповестим:{" "}
            {this.state.blogwin.map((user) => {
              return <d>{user}, </d>;
            })}{" "}
            о вашем решении!{" "}
          </div>
        ...
        <h3>Ваши аукционы</h3>
        {this.props.state.myauction ? (
          <div>
            {this.props.state.myauction &&
              this.props.state.myauction.map((auction) => {
                return (
                  <div>
                    {auction.blogerin.length > 0 ? (
                      <ul>
                        <li>Тема аукциона: {auction.subject}</li>
                        <li>
                          Минимальное количество подписчиков:{" "}
                          {auction.subscribers}
                        </li>
                        <li>Заданный бюджет: {auction.budget}</li>

                        {auction.blogerin.map((blog) => {
                          return (
                            <li>
                              Oткликнулся: {blog.userlogin}{" "}
                              <button
                                className="btn2"
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    selectbloger: (userid, auctionid) =>
      dispatch(selectbloger(userid, auctionid)),
  };
};
