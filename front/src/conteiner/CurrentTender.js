...
class CurrentTender extends React.Component {
  ...
  async componentDidMount() {
    ...
    if (
      this.props.tender.currentTender.initator == localStorage.getItem("id")
    ) {
      this.setState({
        initiator: true,
      });
    ...
    this.timerId = setInterval(() => {
      ...
      this.ws.send(
        JSON.stringify({ currentDate, id: this.props.match.params.tenderID })
      );
    }, 2000);
  ...
  render() {
    return (
      <>
        {this.state.errorBet && <p>{this.state.errorBet}</p>}
        {this.props.tender.currentTender && (
          <div className="torg">
            <h2>{this.props.tender.currentTender.title}</h2>
            <h3>{this.props.tender.currentTender.state}</h3>
            <div>
              <h4>Ход торгов</h4>
              {this.props.tender.currentTender.bets.length && (
                <Bets bets={this.props.tender.currentTender.bets} />
              )}
            </div>

            {this.state.initiator && (
              <Button text="Назад" onClick={this.onClickDeleteHandler} />
            )}
            {!this.state.initiator && this.state.start && (
              <Bet onSubmit={this.onSubmitNewBetHandler} />
            )}
          </div>
        )}
      </>
